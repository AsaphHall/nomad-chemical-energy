...
class CE_NECC_EC_GC(PotentiometryGasChromatographyMeasurement, PlotSection, EntryData):
    ...
    def make_current_voltage_figure(self, date_strings):
        ...
        if self.ph.ph_value is not None:
            ...
            if not np.isnan(self.fe_results.pH_start) and not np.isnan(self.fe_results.pH_end):
                fig.add_trace(
                    go.Scatter(
                        name='pH',
                        x=[self.fe_results.datetime[0].strftime('%Y-%m-%d %H:%M:%S'), self.fe_results.datetime[-1].strftime('%Y-%m-%d %H:%M:%S')],
        ...
        if self.data_file:
            with archive.m_context.raw_file(self.data_file, 'rb') as f:
                ...
                if not self.ph and len(xls_file.sheet_names) < 7:
                    ...
                    ph_start_time = None
                    if 'pH' in data.columns and data.columns.get_loc('pH') + 1 < len(data.columns):
