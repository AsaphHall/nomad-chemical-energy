...
from nomad_chemical_energy.schema_packages.public_showcase_package import (
    GirlsDay_Laboraufzeichnung,
)


class ParsedEMARShowcaseFile(EntryData):
    activity = Quantity(
        type=Activity,
        a_eln=ELNAnnotation(
...
class PublicShowcaseParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger) -> None:
        file_path = mainfile.rsplit('/raw/', maxsplit=1)[-1]
        file_name = file_path.split('/')[-1]

        entry = GirlsDay_Laboraufzeichnung()
        ...
        archive.data = ParsedEMARShowcaseFile(activity=ref)
